    ...
    def inject(self, smali_tree, level,package):
        ...
        for m in self.entries:
            ...
            if ia >= 0:
                ...
                c = m
            # print self.android_api.classes.keys()
...
"move-result-object %s" % r), i + 1, 0)
    ...
    def add_stub_method(self, on, m,parent_class,parent_method,package):
        ...
        if method_type == "constructor":
            ...
            self.__add_stub_inst(stub_class, on, m,parent_class,parent_method)
        elif method_type == "static":
            self.__add_stub_static(stub_class, m,parent_class,parent_method)


    def __add_stub_inst(self, stub_class, on, m,parent_class,parent_method):
        segs = m.rsplit("->", 1)
        ...
        #print m,on,i,method.paras,reg_num
...
Ljava/lang/Exception;->printStackTrace()V"))
    ...
    def     __add_stub_cons2(self, stub_class, m,parent_class,parent_method):
        segs = m.rsplit("->", 1)
...
#invoke-static {v1}, Landroid/net/Uri;->parse(Ljava/lang/String;)Landroid/net/Uri;
    def __add_stub_static(self, stub_class, m,parent_class,parent_method):
        segs = m.rsplit("->", 1)
        method = MethodNode()
        method.set_desc(segs[1])
        #print stub_class,segs[1]
        method.add_access(["public", "static"])
        #print method.name
        para_num = len(method.paras)
        reg_num = method.get_paras_reg_num()
